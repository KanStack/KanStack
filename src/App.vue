...
import { useActionHistory, type HistoryStateSnapshot } from '@/history/useActionHistory'
import { useWorkspace } from '@/composables/useWorkspace'
import { ARCHIVE_COLUMN_NAME, ARCHIVE_COLUMN_SLUG, SHOW_ARCHIVE_COLUMN_SETTING } from '@/utils/archiveColumn'
import type { VisibleBoardCardSelection, WorkspaceCardSelection, WorkspaceSnapshot } from '@/types/workspace'
...
const selectedColumn = computed(() => {
  ...
  return currentBoard.value.columns.find((column) => column.slug === selectedColumnState.value) ?? null
})
...
async function applyHistoryStateSnapshot(state: HistoryStateSnapshot) {
  const snapshot = await invoke<WorkspaceSnapshot>('apply_workspace_snapshot', {
...
async function executeTrackedAction(label: string, perform: () => Promise<HistoryStateSnapshot | null>) {
...
async function createSubBoardFromCurrentBoard() {
  if (!currentBoard.value) {
    return
  }

  const after = await executeTrackedAction('New Sub Board', async () => {
    const result = await appBoardActions.createSubBoard(currentBoard.value!)
    if (!result) {
      return null
    }
...
async function handleColumnReorder(payload: { draggedSlug: string; targetIndex: number }) {
  if (!currentBoard.value) {
    return
  }

  const after = await executeTrackedAction('Reorder Columns', async () => {
    const snapshot = await appBoardActions.reorderColumns(currentBoard.value!, payload.draggedSlug, payload.targetIndex)
    if (!snapshot) {
      return null
    }
...
async function toggleArchiveColumn() {
  ...
  const nextValue = !(currentBoard.value.settings?.[SHOW_ARCHIVE_COLUMN_SETTING] ?? false)
  const after = await executeTrackedAction(nextValue ? 'Show Archive Column' : 'Hide Archive Column', async () => {
    const snapshot = await appBoardActions.saveBoardSettings(currentBoard.value!, {
...
async function toggleSubBoards() {
  ...
  const nextValue = !(currentBoard.value.settings?.['show-sub-boards'] ?? true)
  const after = await executeTrackedAction(nextValue ? 'Show Sub Boards' : 'Hide Sub Boards', async () => {
    const snapshot = await appBoardActions.saveBoardSettings(currentBoard.value!, {
      ...(currentBoard.value!.settings ?? {}),
...
async function handleColumnRename(payload: { name: string; slug: string }) {
  ...
  const after = await executeTrackedAction('Rename Column', async () => {
    const result = await appBoardActions.renameColumn(currentBoard.value!, payload.slug, payload.name)
    if (!result) {
      return null
    }
...
async function handleBoardRename(title: string) {
  ...
  const after = await executeTrackedAction('Rename Board', async () => {
    const result = await appBoardActions.renameBoard(currentBoard.value!, title)
    if (!result?.snapshot) {
      return null
    }
...
function handleCardActivate(payload: { metaKey: boolean; shiftKey: boolean; selection: WorkspaceCardSelection }) {
  selectedColumnState.value = null
  keyboardMoveMode.value = null
...
async function archiveSelectedCards() {
  ...
  const after = await executeTrackedAction('Archive Cards', async () => {
    ...
    for (const [boardSlug, cards] of groupedSelections.entries()) {
      ...
      const snapshot = await appBoardActions.archiveCards(board, cards.map((card) => card.slug))
      if (!snapshot) {
        return null
      }
  ...
  if (selectedCardSlug.value && selectedCards.some((card) => card.slug === selectedCardSlug.value)) {
    requestCloseEditor()
  }
...
async function deleteSelectedCards() {
  ...
  const confirmed = window.confirm(`Delete ${selectedCards.length} selected card${selectedCards.length === 1 ? '' : 's'}?`)
  if (!confirmed) {
    return
  }
...
async function deleteSelectedColumn() {
  ...
  const confirmed = window.confirm(`Delete column "${selectedColumn.value?.name ?? selectedColumnSlug.value}" across all boards?`)
  if (!confirmed) {
    return
  }

  const after = await executeTrackedAction('Delete Column', async () => {
    const result = await appBoardActions.deleteColumn(selectedColumnSlug.value!)
    if (!result) {
      return null
    }

    if (result.blocked) {
      showAppMessage('You can\'t delete that column while it still contains cards anywhere in the workspace. Move, archive, or delete those cards first.')
      return null
    }
...
function handleGlobalKeydown(event: KeyboardEvent) {
  ...
  if (hasPrimaryModifier && event.key.toLowerCase() === 'z' && event.shiftKey) {
    event.preventDefault()
    void redoAction()
    return
  }
  ...
  if (!selectedCardSlug.value) {
    if (event.key === ' ' && boardSelection.selectedCount.value === 1) {
      event.preventDefault()
    ...
    if (keyboardMoveMode.value === 'column' && (event.key === 'ArrowRight' || event.key === 'ArrowLeft')) {
      event.preventDefault()
      void moveSelectedColumnByKeyboard(event.key === 'ArrowRight' ? 1 : -1)
      return
    }

    if (keyboardMoveMode.value === 'card' && ['ArrowRight', 'ArrowLeft', 'ArrowUp', 'ArrowDown'].includes(event.key)) {
      event.preventDefault()
      void moveSelectedCardByKeyboard(event.key as 'ArrowRight' | 'ArrowLeft' | 'ArrowUp' | 'ArrowDown')
      return
    }

    if (selectedColumnSlug.value && (event.key === 'ArrowRight' || event.key === 'ArrowLeft')) {
      event.preventDefault()
    ...
    if (event.key === 'ArrowRight' || event.key === 'ArrowDown') {
      event.preventDefault()
      boardSelection.moveSelection(event.key === 'ArrowRight' ? 'right' : 'next')
      return
    }

    if (event.key === 'ArrowLeft' || event.key === 'ArrowUp') {
      event.preventDefault()
      boardSelection.moveSelection(event.key === 'ArrowLeft' ? 'left' : 'previous')
      return
    }
  }

  if ((event.key === 'Delete' || event.key === 'Backspace') && boardSelection.selectedCount.value > 0) {
    event.preventDefault()
    void deleteSelectedCards()
    return
  }

  if ((event.key === 'Delete' || event.key === 'Backspace') && selectedColumnSlug.value) {
    event.preventDefault()
    void deleteSelectedColumn()
    return
  }

  if (event.key === 'Enter' && boardSelection.selectedCount.value === 1 && !selectedCardSlug.value) {
    event.preventDefault()
    openSelectedCard()
    return
  }
  ...
  if (event.key.toLowerCase() === 'n' && event.shiftKey) {
    event.preventDefault()
    void createSubBoardFromCurrentBoard()
    return
  }
...
async function attachMenuActionListener() {
  ...
  unlistenMenuActions = await listen<{ action: string }>('menu-action', (event) => {
  ...
  switch (action) {
    ...
    case 'rename-selected-column':
      if (selectedColumnSlug.value === ARCHIVE_COLUMN_SLUG) {
        ...
        window.dispatchEvent(new CustomEvent('kanstack:request-rename-column', { detail: { slug: selectedColumnSlug.value } }))
      }
      break
...
function countDescendantBoards(rootSlug: string) {
  ...
  let count = 0
  const stack = [...workspace.value.boardsBySlug[rootSlug].subBoards.map((board) => board.slug)]
  ...
  while (stack.length) {
    ...
    visited.add(slug)
    count += 1
    stack.push(...workspace.value.boardsBySlug[slug].subBoards.map((board) => board.slug))
  }
...
function moveSelectedColumn(delta: -1 | 1) {
  ...
  const visibleColumns = currentBoard.value.columns.filter((column) => column.slug !== ARCHIVE_COLUMN_SLUG)
  const currentIndex = visibleColumns.findIndex((column) => column.slug === selectedColumnSlug.value)
  if (currentIndex === -1) {
    return
  }

  const nextIndex = Math.max(0, Math.min(visibleColumns.length - 1, currentIndex + delta))
  selectedColumnState.value = visibleColumns[nextIndex]?.slug ?? selectedColumnState.value
}
...
async function moveSelectedColumnByKeyboard(delta: -1 | 1) {
  ...
  const snapshot = await appBoardActions.reorderColumns(currentBoard.value, selectedColumnSlug.value, nextIndex)
  if (!snapshot) {
    return
  }
...
async function moveSelectedCardByKeyboard(direction: 'ArrowRight' | 'ArrowLeft' | 'ArrowUp' | 'ArrowDown') {
  if (!workspace.value || boardSelection.selectedCount.value !== 1) {
    return
  }
  ...
  const ownerBoard = workspace.value.boardsBySlug[selectedCard.sourceBoardSlug]
  if (!ownerBoard) {
    return
  }
...
function findCardLocation(board: NonNullable<typeof currentBoard.value>, cardSlug: string) {
...
function getKeyboardCardMoveTarget(
  ...
  if (direction === 'ArrowUp' || direction === 'ArrowDown') {
    ...
    const targetFlatIndex = Math.max(0, Math.min(flatCards.length - 1, location.flatIndex + delta))
    if (targetFlatIndex === location.flatIndex) {
      return null
    }

    const targetCard = flatCards[targetFlatIndex]
    let targetIndex = direction === 'ArrowUp' ? targetCard.cardIndex : targetCard.cardIndex + 1

    if (targetCard.section.slug === location.section.slug && location.cardIndex < targetIndex) {
      targetIndex -= 1
    }

    return {
      cardSlug: targetCard.slug === location.section.cards[location.cardIndex]?.slug ? location.section.cards[location.cardIndex].slug : board.columns[location.columnIndex].sections[location.sectionIndex].cards[location.cardIndex].slug,
  ...
  const targetFlatIndex = Math.min(location.flatIndex, targetFlatCards.length)

  if (!targetFlatCards.length) {
    const matchingSection = location.section.slug
      ? targetColumn.sections.find((section) => section.slug === location.section.slug)
      : targetColumn.sections.find((section) => section.slug === null)

    return {
      cardSlug: board.columns[location.columnIndex].sections[location.sectionIndex].cards[location.cardIndex].slug,
...
function flattenColumnCards(column: NonNullable<typeof currentBoard.value>['columns'][number]) {
...
watch(currentBoard, (board) => {
  ...
  if (!board.columns.some((column) => column.slug === selectedColumnState.value)) {
    selectedColumnState.value = null
  }
})
...
<template>
  <div class="app-shell">
    <AppHeader
      ...
      :is-creating-sub-board="appBoardActions.isCreatingSubBoard.value"
      @create-sub-board="createSubBoardFromCurrentBoard"
